  ...
  def buildFromCreditBalances(cbs: Seq[CreditBalance]): CreditHistory = {
    val history = cbs.map { cb: CreditBalance =>
      if (cb.transactionType == TransactionType.Debit)
        Debit(cb.transactionId, cb.createdAt, Amount(cb.value))
      else
        Credit(cb.transactionId, cb.createdAt, Amount(cb.value), cb.reason.get.toString(), CreditIssuer(cb.adminUserId.get.toString, Set()))
    }
