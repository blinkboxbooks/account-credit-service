Given(/^a customer with (\d+(?:\.)?(?:\d*)) in GBP of credit$/) do |credit_balance|
...
When(/^I credit the customer (-?\d+(?:\.)?(?:\d*)) in GBP with the reason: (.*)$/) do |credit_amount, reason|
  post_admin_account_credit(last_admin_user.access_token, credit_amount, reason,
                            user_id_of(last_public_user), new_request_id)
end

When(/^I try to credit the customer (\d+(?:\.)?(?:\d*)) in GBP using the same requestId as before$/) do |credit_amount|
